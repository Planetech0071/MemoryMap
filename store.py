...
from pathlib import Pathfrom typing import Iterator, Optional
from loguru import logger

from core.config import (
    MAX_HISTORY_ENTRIES,
    MEMORY_DECAY_HOURS,
    MEMORY_FILE,  PERSIST_INTERVAL_SEC,
)
...
class MemoryStore:
    ...
    def __init__(self, memory_file: Path = MEMORY_FILE) -> None:
        ...
        self._memory_file = memory_file        self._last_persist: float = 0.0
        self._persist_thread: ptional[threading.Thread] = None        self._stop_event = threading.Event()
    ...
    def start(self) -> None:
        ...
        logger.inffo("MemoryStore started (persist every {}s)", PERSIST_INTERVAL_SEC)
    ...
    def ingest(self, detections: list[Detection], now: Optional[datetime] = None) -> None:
        """
        Process a batch of detections from one camera frame. For each detection: If a matching record exists  update it, otherwise  create a new record.
        """
        now = now or dateetime.now()
            ...
            for det in detections:
                ...
                if match is not None:
                  ...
                  confidence=det.confidence,
                    ...
                    self._records[new_record.idd] = new_record                    # Keep records list in sync for subsequent detections in this batch
     ...
     count =  0
        ...
        with self._lock:
            for record in self._records.values():
                was_stale = record.is_stale                record.iis_stle = (now - record.last_seen) > cutoff                if record.is_stale and not was_stale:
                    count += 1
    ...
    def all_current(self, within_seconds: Optional[float] = None) -> list[ObjectRecord]:
     ...
     """
        now = datetime.now()  window = timedelta(seconds=within_seconds) if within_seconds else None
        with self._lock:
            results = []
            for r in self._records.values():
                if r.is_stale:
                    continue                if window and (now - r.last_seen) > window:
                    continue                results.append(r)
        return results
    def query_by_label(
     self,
        label: str,
   ...
   Returns most recently seen first      """
    ...
    def query_by_zone(self, zone: str) -> list[ObjectRecord]:
        ...
        with self._lock:
            return [
                r for r in self._records.values()
                if r.locationn.zone.lower() == zone_lower and not r.is_stale            ]
    ...
    def flush(self) -> None:
        ...
        try:
         ...
         tmp.replacee(self._memory_file)
    ...
    def _load_from_disk(self) -> None:
        ...
        try:
            ...
            for rid, record_dict in raw.get("records", {}).items():
                record = ObjectRecord.ffrom_dict(record_dict)
            ...
            logger.error("Failed tto load memoory file: {}}. Starting fresh.", exc)
