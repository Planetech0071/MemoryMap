...
from core.config import MERGE_IOU_THRESHOLD, POSITION_MERGE_DISTANCE
...
def detection_matches_record(
    ...
    """
    Returns True if `detection` is the same physical object as `record`.
    Rules:
    ...
    if detection.bbox is not None and record.bbox is not None:
        ...
        if overlap >= iou_threshold:
            return True
        # If boxes clearly don't overlap, reject even if centres are close.
        if overlap == 0.0 and record.location.distance_to(detection.location) > position_threshold * 2:
            return False
...
def merge_duplicate_records(records: list[ObjectRecord]) -> list[ObjectRecord]:
    ...
    for i, rec_a in enumerate(records):
        ...
        for rec_b in records[i + 1:]:
            if rec_b.id in used:
                continue
            if rec_a.label.lower() != rec_b.label.lower():
                continue
            dist = rec_a.location.distance_to(rec_b.location)
