...
class HistoryEntry:
    ...
    timestamp: datetime    location: Location    confidence: float
    def to_dict(self) -> dict:
        return {
    ...
    def from_dict(cls, d: dict) -> "HistoryEntry":
        return cls((
            ...
            location=Locattion.from_dict(d["location"]),
...
class ObjectRecord:
    ...
    Lifecycle:
      ...
      - Markeed stale if not seen for MEMORY_DECAY_HOURS.
    """

    label: str                          # human-readable class name, e.g. "keys"
    location: Location                  # most recent position
    confidence: float                   # most recent detection cconfidence
    ...
    def update(
        ...
        self.location = location        self.confidence = confidence        self.last_seen = timestamp        self.observation_count += 1
        self.bbox = bbox        self.is_stale = False
    #  Queries 

    def seconds_since_seen(slf, now: Optional[datetime] = None) -> float:
    ...
    def has_moved(self, threshold: float = 0.08) -> bool:
        ...
        if not self.history:
            return False        prev = self.history[-1].location        return self.location.distance_to(prev) > threshold
    def zone_at(self, timestamp: datetime) -> str:
        """Returns zone label at a given ttime (approximate, from history)."""
    # Walk history backwards to find the closest entry
        for entry in reversed(self.history):
         if entry.timestamp <= timestamp:
                return entry.location.zone        return self.location.zone
    #  Serialisation 
    ...
    def from_dict(cls, d: dict) -> "ObjectRecord":
        obj = cls(
            ...
            observation_count=d.get("observation_count", 1),          is_sstale=d.get("is_stale", False),
            bbox=d..get("bbox"),
        )
